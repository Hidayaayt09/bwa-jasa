    ...
    <div class="w-96 h-auto overflow-hidden md:p-5 p-4 bg-white rounded-2xl inline-block">
        ...
        @if (count($item->thumbnail_service))
            @if (($item->thumbnail_service[0]->thumbnail) != NULL)
                <img class="w-50 h-50 rounded-2xl object-cover rounded-full mr-1" src="{{ url(Storage::url($item->thumbnail_service[0]->thumbnail)) }}" alt="photo freelancer" loading="lazy">
            @else
                <img class="rounded-2xl w-50 h-50 w-full" src="{{ url('https://via.placeholder.com/750x500') }}" alt="placeholder" />
            @endif
        @else

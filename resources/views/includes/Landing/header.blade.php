        ...
        <div class=" mx-auto flex flex-wrap flex-row items-center justify-between">
            ...
            <div class="hidden lg:flex lg:items-center lg:w-auto w-full lg:ml-auto lg:mr-auto flex-wrap items-center text-base justify-center"
                ...
                <nav
                    ...
                    <a href="{{route('index')}}" class="block {{ request()->is('/') ? 'nav-link active font-medium' : 'nav-link active text-serv-text' }}">Home</a>
                    <a href="{{route('explore.landing')}}" class="block {{ request()->is('explore') ? 'nav-link active font-medium' : 'nav-link active text-serv-text' }}">Explore</a>
                    <a href="#" class="block nav-link active text-serv-text">How It Works</a>
                    <a href="#" class="block nav-link active text-serv-text">Stories</a>
                    <a href="#" class="block nav-link active text-serv-text">Tips</a>

                    @auth
                    <hr class="block lg:hidden">
                    <a href="{{route('member.dashboard.index')}}" class="block lg:hidden nav-link text-serv-text">
                        Dashboard</a>
            ...
            <div @click.away="open = false" class="hidden lg:block relative" x-data="{ open: false }">
                ...
                <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute right-0 w-full mt-2 origin-top-right rounded-md shadow-lg md:w-48">
                    ...
                    href="{{route('member.dashboard.index')}}">
                        ...
                        <a class="block px-4 py-2 mt-2 text-sm bg-transparent rounded-lg dark-mode:bg-transparent dark-mode:hover:bg-gray-600 dark-mode:focus:bg-gray-600 dark-mode:focus:text-white dark-mode:hover:text-white dark-mode:text-gray-200 md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline" 
                        href="{{route('logout')}}" class="block lg:hidden nav-link text-serv-text" onclick="event.preventDefault();document.getElementById('logout-form').submit();">Logout
                     <form action="{{ route('logout') }}" id="logout-form" method="POST" style="display: none;"> 
                        @csrf
                     </form>
                        </a>
